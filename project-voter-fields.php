...
function iddt_meta_projectvoting_callback( $post ){
  ...
  <div>
    <div class="meta-row">
      <div class="meta-th">
        <label for="produktart" class="iddt-row-title"><?php _e( 'Produktart', 'project-voter');?> Produktart</label>
      </div>
      <div class="td">
        <input type="text" class="iddt-row-content" name="produktart" id="produktart"
        value="<?php if( !empty ($iddt_stored_meta['produktart'])) {
          echo ect_attr( $iddt_stored_meta['produktart'][0]);
        }?> "/>
      </div>
    </div>
...
function iddt_meta_save ( $post_id){
  ...
  if(isset($_POST['produktart'])){
    update_post_meta($post_id, 'produktart', sanatize_text_field( $_POST['produktart']));
  }
}
